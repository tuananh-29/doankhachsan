...
const EMOJIS = { Standard:'🌿', Deluxe:'🌊', Suite:'🏙️', Presidential:'👑' }
...
export default function AdminRooms() {
  ...
  const save = async () => {
    ...
    setSaving(true)
    const payload = { ...form, price: parseInt(form.price), amenities: form.amenities.split(',').map(s => s.trim()).filter(Boolean), emoji: EMOJIS[form.type] || '🛏️' }
    try {
      ...
      <div style={S.wrap}>
        {loading ? <p style={S.empty}>Đang tải...</p> : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr>{['Phòng','Loại','Tên','Giá/đêm','Trạng thái','Thao tác'].map(h => <th key={h} className="admin-th">{h}</th>)}</tr></thead>
            <tbody>
              {rooms.map(r => (
                <tr key={r.id}>
                  <td className="admin-td">{r.emoji} {r.number}</td>
